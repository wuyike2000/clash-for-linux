...
## 判断是否提供了本地的config.yaml文件，如果提供了就使用该文件，否则再根据链接下载
if [[ -f "$Server_Dir/config.yaml" ]]; then
    echo "使用本地config.yaml文件"
    ConfigFile="$Server_Dir/config.yaml"
else
    ## Clash 订阅地址检测及配置文件下载
    # 检查url是否有效
    echo -e '\n正在检测订阅地址...'
    ...
    ConfigFile="$Temp_Dir/clash_config.yaml"
fi
...
## Clash 配置文件重新格式化及配置
# 取出代理相关配置
#sed -n '/^proxies:/,$p' $ConfigFile > $Temp_Dir/proxy.txt
sed -n '/^proxies:/,$p' $ConfigFile > $Temp_Dir/proxy.txt
